...
shinyServer(function(input, output) {
  ...
  datasetInput <- reactive({
    ...
    p <- plot_ly(year.trimmed, x = ~Group.1, y = ~x, type = 'bar', name = 'Average GPA', color = ~Group.1) %>%
      layout(yaxis = list(title = 'Average GPA', range = c(0, 4)), margin = list(b = 180), xaxis = x)

    return(p)

  })

  # sends list of years to select input
  output$select <- renderUI({
    if (is.null(input$text)) {
      return()
    } else {
      filteredCourses <- filter(data, Class == toupper(input$text))
      selectInput("select", label = h3("Select Year"),
                  choices = c(unique(as.character(filteredCourses$Year))))
    }
  })
